...
class Example(QMainWindow):
	def __init__(self):
		...
		self.current_pos, D_X, D_Y, address_obg = get_obj(self.address)
	...
	def save_map(self, point=False):
		...
		if point and address:
			self.current_pos, D_X, D_Y, address_obg = get_obj(address)
			self.current_pos = list(map(float, self.current_pos.split(' ')))
			self.delta = min(D_X, D_Y)
			self.point = f'{self.current_pos[0]},{self.current_pos[1]}'
			self.textEdit.clear()
			self.textEdit.insertPlainText(address_obg)
		obj_coodrinates, d_x, d_y, address_obg = get_obj(f'{self.current_pos[0]},{self.current_pos[1]}')
